...
HRESULT Graphic::Initialize(ID3D12Device* device, D3D12_DESCRIPTOR_RANGE& descRangeSRV, int cbnum)
{
	HRESULT result = Initialize(device, descRangeSRV, cbnum, L"BasicVS.hlsl", L"BasicPS.hlsl", L"BasicGS.hlsl");
	return result;
}

HRESULT IF::Graphic::Initialize(ID3D12Device* device, D3D12_DESCRIPTOR_RANGE& descRangeSRV, int cbnum, LPCWSTR vs, LPCWSTR ps, LPCWSTR gs)
{
	HRESULT result;

	RootParam root(descRangeSRV, 1, cbnum);
...
HRESULT IF::Graphic::InitializeParticle(ID3D12Device* device, D3D12_DESCRIPTOR_RANGE& descRangeSRV)
{
	...
	RootParam root(descRangeSRV, 1, 2);
...
void IF::Graphic::BlendMode(ID3D12GraphicsCommandList* commandList, Blend blend)
{
